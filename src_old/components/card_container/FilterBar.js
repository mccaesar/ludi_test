...
class FilterBar extends Component {
  ...
  render() {
    if (this.props.containerType === 'recommended') {
      //return null for certain containerTypes
      return null;
    }
    ...
    switch (this.props.containerType) {
      case 'resultsContainer':
        if (this.props.numResults === 1) {
          resultsStatement = <div> {this.props.numResults} result found. </div>;
        } else {
          resultsStatement = (
    ...
    switch (this.props.containerType) {
      case 'resultsContainer':
        ...
        optionsList.push(
          <option value="alphabeticalReverse">Alphabetical (Z-A)</option>
        );
        optionsList.push(<option value="date">Date added (newest)</option>);
        optionsList.push(
          <option value="dateReverse">Date added (oldest)</option>
        );
        break;
      case 'saved':
        optionsList.push(
