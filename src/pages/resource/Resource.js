  ...
  saveResources = () => {
    ...
    dataRef.once('value').then((dataSnapshot) => {
      if (dataSnapshot.hasChild(auth.currentUser.uid)) {
        ...
        var pathToUserSavedComplete = "users/" + auth.currentUser.uid + "/savedResources"
        if (dataSnapshot.hasChild(pathToUserSavedRelative)) {
          ...
          let savedResourcesRef = firebase.database().ref(pathToUserSavedComplete);
        ...
        let currUserRef = firebase.database().ref("users/" + auth.currentUser.uid);
        let userObject = {
      ...
      return (
        <div class="page-container">
          <div className="App" class="content-wrap">
            ...
            <Helmet><title>LUDI | {this.state.data.Title}</title></Helmet>
            <Header />
            <Overview
            ...
            <Guidance potentialUseCase={this.state.data.PotentialUseCase} pros={this.state.data.Pros} cons={this.state.data.Cons} />
            <CardContainer containerType="recommended" searchInfo={this.state.searchInfo}/>
          </div>
          <Footer />
        </div>
